...
namespace pthash {
    ...
    static char pthash_static_coda[280] =
    ...
    const std::string pthash_static_header_coda() {
        // size: 177
        return pthash_static_coda;
    }
    const std::string pthash_static_lookup_coda(const std::string key_type = "uint64_t") {
        // size: 177 + 83
        char func[128];
        snprintf(func, sizeof(func), "%s pthash_unordered_lookup(const %s key) {\n"
...
#endif
                 , key_type.c_str());
        std::string result (pthash_static_coda);
        return result + std::string(func);
    }
