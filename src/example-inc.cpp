...
int main() {
    ...
    /* first check unordered lookup with the external index table */
    for (pthash::bucket_id_type i = 0; i < ARRAY_SIZE(testkeys); i++) {
        ...
        /* then check the builtin ordered pthash_lookup with the included index table */
        pthash::bucket_id_type sorted_p = pthash_lookup(key);
        std::cout << i << ": pthash_lookup(" << key << ") = " << sorted_p << "\n";
        if (sorted_p != i) {
            if (i == ARRAY_SIZE(testkeys) - 1) {
                std::cout << " (false-positive)\n";
                continue; // should not be found
            }
            std::cerr << "ERROR: " << sorted_p << " != " << i << " (index_table["
                      << testkeys[i].result << " ])\n";
            errs++;
        }
