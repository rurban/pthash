...
void test(std::vector<uint64_t> &keys, pthash::build_configuration &config) {
    ...
    for (uint64_t i = 0; i < 10; ++i) {
        std::cout << i << ": f(" << keys[i] << ") = " << f(keys[i]) << std::endl;
    }
    ...
    for (bucket_id_type i = 0; i < f.num_keys(); ++i) {
        sorted_keys_file << key_table[i] << std::endl;
    }
    ...
    out.open(output_filename_c, std::ios::app);
    out << ";\n"
        "  const bucket_id_type p = pthash_unordered_lookup(key);\n"
        "  return index_table[p];\n}\n";
    out.close();
